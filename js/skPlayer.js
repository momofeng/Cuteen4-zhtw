    ...
    function skPlayer(option) {
        ...
        if (this.type === 'file') {
            ...
            .catch(function (error) {
                console.error('歌單拉取失敗！ 錯誤碼：' + error);
            });
        }
    }
            ...
            if (this.type === 'cloud') {
                Util.ajax({
                    ...
                    fail: function fail(status) {
                        console.error('歌曲拉取失敗！ 錯誤碼：' + status);
                    }
                });
            }
        }
