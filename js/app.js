...
<div class="hljs-control hljs-title">
            <div type="button" data-toggle="tooltip" data-placement="bottom" data-original-title="折行" class="hljs-control-toggle-break-line">
                    ...
                    <div id=` + copyBtnID + ` type="button" data-toggle="tooltip" data-placement="bottom" data-original-title="複製" class="hljs-control-copy">
            ...
            clipboard.on('success', function (e) {
                Toastify({
                    duration: 1500,
                    position: 'center',
                    text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#check-circle"></use></svg>覆製成功!',
            ...
            clipboard.on('error', function (e) {
                Toastify({
                    duration: 1500,
                    position: 'center',
                    text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#x-circle"></use></svg>覆製失敗!',
        ...
        fm.onsubmit = function (e) {
            ...
            axios.post(action, fmData)
                .then(function (response) {
                    ...
                    const convert = parser.parseFromString(response.data, "text/html");//字符串轉換為dom
                    const error = new RegExp('Typecho_Widget_Exception');
                    if (error.test(response.data)) {
                        Toastify({
                            duration: 3000,
                            position: 'center',
                            text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#x-circle"></use></svg>評論失敗!回傳參數錯誤!',
                            backgroundColor: "var(--bs-danger)",
                            className: "info",
                        }).showToast();
                        ...
                        console.log('回覆提交成功！');
                        document.getElementById("comment-textarea").value = "";//清空回覆
                        window.TypechoComment.cancelReply();//復位回覆框
                        ...
                        const newList = convert.getElementsByClassName('comment-list')[0].outerHTML;//獲取新列表
                        console.log(oldList)
                        if (oldList.length > 0) {
                            oldList[0].remove();//移除舊列表
                        }
                        document.getElementById('comment-list').insertAdjacentHTML('afterbegin', newList);//插入新列表
                        Toastify({
                            duration: 3000,
                            position: 'center',
                            text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#check-circle"></use></svg>評論成功!',
                            backgroundColor: "var(--bs-success)",
                            className: "info",
                        }).showToast();//提示成功
                        document.getElementById(newId).scrollIntoView({behavior: "smooth"});//滾動到對應錨點
                    }
                })
                .catch(function (error) {
                    Toastify({
                        duration: 3000,
                        position: 'center',
                        text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#x-circle"></use></svg>評論失敗!提交錯誤!',
                        backgroundColor: "var(--bs-danger)",
                        className: "info",
                    }).showToast();
                    console.log(error);
        ...
        if (like !== null) {
            ...
            if (dataID === starValue) {
                ...
                Toastify({
                    duration: 3000,
                    position: 'center',
                    text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#check-circle"></use></svg>您已點過贊啦!',
                ...
                axios.post(Config.homeUrl + '/action/cuteen?content', {
                    ...
                    .then(function (response) {
                        ...
                        Toastify({
                            duration: 3000,
                            position: 'center',
                            text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#check-circle"></use></svg>點贊成功！感謝支持~',
                            backgroundColor: "var(--bs-success)",
                            className: "info",
                        }).showToast();//點贊成功！感謝支持~
                        return '';
                    ...
                    .catch(function (error) {
                        Toastify({
                            duration: 3000,
                            position: 'center',
                            text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#x-circle"></use></svg>點贊失敗!提交錯誤!',
                            backgroundColor: "var(--bs-danger)",
                            className: "info",
                        }).showToast();
                        console.log(error);
    ...
    startSearch: function (item) {
        const c = item.value;
        if (!c || c === '') {
            item.setAttribute('placeholder', '你還沒有輸入任何信息');
            return;
        }
        ...
        link.onclick = function (e) {
            if (href !== undefined) {
                axios.interceptors.request.use(config => {
                    ...
                    link.insertAdjacentHTML('beforeend', '<button class="btn btn-primary col-3 mx-auto rounded-pill NextLoading" type="button" disabled>加載中...</button>');
                    return config
                }, error => {
                ...
                axios({
                    ...
                    if (error.test(response.data)) {
                        Toastify({
                            duration: 3000,
                            position: 'center',
                            text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#x-circle"></use></svg>加載失敗!回傳參數錯誤!',
                            backgroundColor: "var(--bs-danger)",
                            className: "info",
                        }).showToast();
                        console.log(response.data);
                    } else {
                        const list = convert.getElementsByClassName('article');//獲取新列表
                        ...
                        if (newHrefPosition!==undefined) {
                            link.insertAdjacentHTML('beforeend', '<button id="NextButton" onclick="Cuteen.ajaxNext()" class="btn btn-primary col-3 mx-auto rounded-pill">點擊加載更多</button>')
                    ...
                    Toastify({
                        duration: 3000,
                        position: 'center',
                        text: '<svg class="icon icon-20" aria-hidden="true"><use xlink:href="#x-circle"></use></svg>加載失敗!參數錯誤',
                        backgroundColor: "var(--bs-danger)",
                        className: "info",
                    }).showToast();
                    console.log(error);
                });
                e.stopPropagation(); // 阻止冒泡到父級的點擊事件
            }
            return false;
        }
        return false;
